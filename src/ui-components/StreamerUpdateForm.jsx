...
export default function StreamerUpdateForm(props) {
  const {
    id,
    streamer,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...streamerRecord };
    ...
    setChannelcreated(cleanValues.channelcreated);
    ...
    setDRlink(cleanValues.DRlink);
    setErrors({});
  };
  const [streamerRecord, setStreamerRecord] = React.useState(streamer);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Streamer, id) : streamer;
      setStreamerRecord(record);
    };
    queryData();
  }, [id, streamer]);
  React.useEffect(resetStateValues, [streamerRecord]);
  const validations = {
    username: [],
    name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Streamer.copyOf(streamerRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "StreamerUpdateForm")}
    >
      <TextField
        label="Username"
        isRequired={false}
